...
(define (numbersXPtsList funString range rangeY auxs) ; temporary function, to fix with global calculations of axes and ticks
  (let ((ticksX (setTicksX range))
    ...
    (let ((numbersPointFun (lambda (x) (list x (- limY (/ (width rangeY) 12)))))) ; function that adjusts the position of the number based on the position of the tick
      ;; can be probably defined apart (I need one for x and one for y)
      (map numbersPointFun ticksX))))

(define (numbersYPtsList funString range rangeY auxs) ; temporary function, to fix with global calculations of axes and ticks
...
(define (numbersXPoints funString range rangeY auxs)
  (let ((numbersPtLst (numbersXPtsList funString range rangeY auxs))
	(pointFun
	 (lambda (x)
	   (list 'with "text-at-halign" "center"
		 (list 'text-at (number->string (car x))
		       (list->pt (car (rescalePairsRef (list x) (ptlist funString range)))))))))
...
(define (numbersYPoints funString range rangeY auxs)
  (let ((numbersPtLst (numbersYPtsList funString range rangeY auxs))
	(pointFun (lambda (x)
