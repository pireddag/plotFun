...
;; ticks
;; a relative rescaling function (do I have to define just one? Do I need to rescale relative to the function values only?)
;;
...
(define (numbersXPtsList auxs) ; temporary function, to fix with global calculations of axes and ticks
  (let* ((rangeX (cdr (assoc "rangeX" auxs)))
	 ...
	 (numbersPointFun (lambda (x) (list x (- limY (/ (width rangeY) 12))))))
      ;; function that adjusts the position of the number based on the position of the tick
